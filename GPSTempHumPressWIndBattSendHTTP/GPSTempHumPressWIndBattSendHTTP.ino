...
uint8_t interruptPinWind = D3;
float lastWindChk;
float windSpeed;
float lastWindClick = millis();

volatile int windClickNo = 0;
void ICACHE_RAM_ATTR clickWind ();

void clickWind(){
  if( (millis() - lastWindClick) > 100)
  {
    windClickNo++;
...
void loop() {
  ...
  //if (now - lastMsg > 20000) {
    ...
    snprintf (msg, MSG_BUFFER_SIZE, "Temperatura #%lf Humedad #%1f  Presion #%1f Voltaje #%1f viento #%1f ", temperature, humidity, pressure, Rvalue,speedWind);
    Serial.println(msg);
    ...
    // Make a HTTP request:
    sprintf (telemetria, "tem=%f&pre=%f&alt=%f&hum=%f&lat=%f&lon=%f&vol=%f&vel_viento=%f", temperature, (pressure / 100.0F), bme.readAltitude(SEALEVELPRESSURE_HPA), humidity, lat, lon, Rvalue,speedWind);
